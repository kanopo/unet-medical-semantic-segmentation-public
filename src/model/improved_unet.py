...
class IUNET(nn.Module):
    def __init__(self, conv_dropout=0.0, final_dropout=0.0):
        ...
        self.encoder3 = Encoder(in_channels=32, out_channels=64)
        self.encoder4 = Encoder(in_channels=64, out_channels=128)
        self.encoder5 = Encoder(in_channels=128, out_channels=256)
        self.encoder6 = Encoder(in_channels=256, out_channels=512)
        ...
        self.decoder6 = Decoder(in_channels=32, out_channels=16)
        """
        Final conv
        """
        self.convolution = nn.Conv2d(
            in_channels=16, out_channels=1, kernel_size=1  # binary segmentation
        )
    ...
    def forward(self, inputs):
        ...
        double_conv5, p5 = self.encoder5(p4)
        double_conv6, p6 = self.encoder6(p5)

        """
        Bottom
        """
        bottom = self.bottom(p6)

        """
        Decoder
        """
        decoder1 = self.decoder1(bottom, double_conv6)
        decoder2 = self.decoder2(decoder1, double_conv5)
        decoder3 = self.decoder3(decoder2, double_conv4)
        decoder4 = self.decoder4(decoder3, double_conv3)
        decoder5 = self.decoder5(decoder4, double_conv2)
        decoder6 = self.decoder6(decoder5, double_conv1)

        """
        Final conv
        """
        output = self.convolution(decoder6)
