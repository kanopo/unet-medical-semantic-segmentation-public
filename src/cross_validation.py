...
#         )
        ...
        get_prediction(f"./fold_{fold}_weights.pth", names=test_ids, images=test_images, labels=test_labels, device=DEVICE, path="../results/")
...
#
    ...
    data: pd.DataFrame = pd.DataFrame(columns=["image_path", "mean", "variance", "standard_deviation", "5_percetile", "10_percentile","50_percentile","90_percentile", "95_percentile"])
    ...
    for dir in result_dirs:
        ...
        extracted_femur_handmade.save(os.path.join("../results", dir, "extracted_femur_handmade.png"))
        extracted_femur_model.save(os.path.join("../results", dir, "extracted_femur_model.png"))
        ...
        pd.DataFrame(distribution_handmade).to_csv(f"../results/{dir}/handmade_data.csv", index=False)
        pd.DataFrame(distribution_model).to_csv(f"../results/{dir}/model_data.csv", index=False)
        ...
        data.loc[len(data)] = [f"./{dir}/extracted_femur_model.png",
                     mean_model,
                     variance_model,
                     std_model,
                     perc_model[0],
